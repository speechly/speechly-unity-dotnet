...
Constructing SpeechlyClient requires an `appId` from [Speechly Dashboard](https://api.speechly.com/dashboard/) to connect to a configuration that determines if the NLU engine should detect intents and keywords (entities) from the spoken utterance in addition to speech-to-text (ASR). Setting `manualUpdate: true` postpones SpeechlyClient's callbacks (OnSegmentChange, OnTranscript...) until you manually run `SpeechlyClient.Update()`. This enables you to use Unity API in callbacks, which need to be executed in the main Unity thread.
